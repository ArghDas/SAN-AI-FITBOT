...
API_KEY = "AIzaSyCSryri9Mgo2oqump05gw0NHOYr3FIrml0"
...
try:
    model = genai.GenerativeModel(
        ...
        system_instruction="You are SAN, a fitness chatbot and nutrition specialist  dedicated to helping users achieve their health and fitness goals through personalized workout plans, diet plans, dietary advice, progress tracking, motivational support, and educational resources, promoting a balanced and sustainable approach to health and wellness.",
    )
...
# FitnessAgent class
class FitnessAgent:
    def __init__(self, google_gemini_api_key: str, nut_api_key: str):
    ...
    def get_nutritional_info(self, query: str) -> dict:
        ...
        try:
            response = requests.get(api_url, timeout=10, headers={'X-Api-Key': self.nut_api_key})
            response.raise_for_status()  # Raise an HTTPError for bad responses
            return response.json()
        except requests.exceptions.HTTPError as http_err:
            return {"Error": response.status_code, "Message": str(http_err)}
    ...
    def calculate_tdee(self, bmr: float, activity_level: str) -> float:
        ...
        Parameters:
            ...
            activity_level (str): Activity level ranging from '1' to '5'

        Returns:
            float: Total Daily Energy Expenditure (TDEE)
        """
        # Define activity factors for various activity levels
        activity_factors = {
            ...
            '4': 1.725,  # Very active (hard exercise/sports 6–7 days/week)
            '5': 1.9,  # Super active (very hard exercise/physical job)
        }

        # Validate activity level input
        if activity_level not in activity_factors:
            raise ValueError(f"Invalid activity level '{activity_level}'. Please choose from '1' to '5'.")

        # Calculate TDEE
        tdee = bmr * activity_factors[activity_level]

        return round(tdee, 2)  # Returning rounded TDEE for better readability
...
def main():
    api_key = "AIzaSyCSryri9Mgo2oqump05gw0NHOYr3FIrml0"
    nut_api_key = "RFjfXsYcF9zbcglzfpBmWw==B7P5YwLWYkcjCc4q"

    chatbot = ChatBot(api_key=api_key)
    fitness_agent = FitnessAgent(google_gemini_api_key=api_key, nut_api_key=nut_api_key)
    chatbot.start_conversation()

    print("Welcome to SAN, the World's Most Advanced Fitbot. Type 'Quit' to exit.")
    print("Inspired By: Chris Bumstead aka Five Time Mr. Olympia")
    print("We will start by making a diet plan according to your goal (e.g., Muscle Gain, Maintain Weight, Weight Loss and Fat Loss). Kindly enter your diet plan and goal.")
    ...
    while True:
        ...
        try:
            if "diet plan" in user_input.lower() or "muscle gain" in user_input.lower() or "weight loss" in user_input.lower() or "fat loss" in user_input.lower() or "maintain weight" in user_input.lower():
                ...
                if "muscle gain".lower() in user_input.lower() or "gain weight".lower() in user_input.lower():
                    ...
                    goal = "weight loss"
                elif "fat loss".lower() in user_input.lower():
                    goal = "fat loss"
                ...
                diet_preference = input("Diet Preference (Veg/Non-Veg): ").lower()
                ...
                diet_plan_prompt = f"{user_profile}\n\nBased on the above information, create a detailed {days}-day {diet_preference} diet plan for the user to achieve their goal of {goal}."
                print(diet_plan_prompt)
                print(f"BMI: {bmi}")
                print(f"BMR: {bmr}")
                print(f"TDEE (Total Daily Energy Expenditure): {tdee}")
